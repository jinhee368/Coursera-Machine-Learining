...
yd=eye(num_labels);
Y=yd(y,:); %Convert y vector into Y matrix

J=sum(sum((-1/m)*[Y.*log(h)+(1-Y).*log(1-h)]));

Theta1s=Theta1(:,2:end); %Excluding Theta_0 terms
Theta2s=Theta2(:,2:end); %Excluding Theta_0 terms

Regularization_Term=(lambda/(2*m))*(sum(sum(Theta1s.^2))+sum(sum(Theta2s.^2)));
